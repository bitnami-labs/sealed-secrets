...
func TestSeal(t *testing.T) {
	...
	for i, tc := range testCases {
		t.Run(fmt.Sprint(i), func(t *testing.T) {
			solveNamespace := func() (string, bool, error) {
			...
			outbuf := bytes.Buffer{}
			if err := Seal(solveNamespace, outputFormat, &inbuf, &outbuf, scheme.Codecs, key, tc.scope, false, "", ""); err != nil {
...
func mkTestSealedSecret(t *testing.T, pubKey *rsa.PublicKey, key, value string, opts ...mkTestSecretOpt) []byte {
	...
	if err := Seal(solveNamespace, outputFormat, inbuf, &outbuf, scheme.Codecs, pubKey, ssv1alpha1.DefaultScope, false, "", ""); err != nil {
		t.Fatalf("seal() returned error: %v", err)
	}
...
func TestMergeInto(t *testing.T) {
	...
	merge := func(t *testing.T, newSecret, oldSealedSecret []byte) *ssv1alpha1.SealedSecret {
		...
		if err := SealMergingInto(solveNamespace, outputFormat, buf, f.Name(), scheme.Codecs, pubKey, ssv1alpha1.DefaultScope, false); err != nil {
			t.Fatal(err)
		}
