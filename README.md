...
### How to retrieve the SBOM for an image?

Each container image includes an SPDX JSON SBOM attached
as a [BuildKit attestation](https://docs.docker.com/build/attestations/sbom/).
Binary release archives also include SPDX JSON SBOMs
generated by [syft](https://github.com/anchore/syft).

You can scan the container image SBOM directly with
[trivy](https://github.com/aquasecurity/trivy):

```bash
# scan the GHCR image using the attached SBOM
trivy image ghcr.io/bitnami-labs/sealed-secrets-controller:latest
```

To inspect the raw SBOM:

```bash
# extract the SBOM attestation from the image
docker buildx imagetools inspect \
  ghcr.io/bitnami-labs/sealed-secrets-controller:latest \
  --format '{{ json .SBOM }}'
```

For binary releases, the SBOM files are available as
`*.sbom.json` alongside the release archives on the
[GitHub Releases](https://github.com/bitnami-labs/sealed-secrets/releases)
page:

```bash
# scan a binary release SBOM
trivy sbom kubeseal-*.sbom.json
```
