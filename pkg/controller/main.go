...
func watchKeyRegistry(ctx context.Context, client kubernetes.Interface, keyRegistry *KeyRegistry, namespace, keyOrderPriority string) error {
	...
	for event := range watcher.ResultChan() {
		...
		case watch.Modified:
			slog.Info("Secret modified", "namespace", secret.ObjectMeta.Namespace, "name", secret.ObjectMeta.Name)
			// todo support updating
		case watch.Deleted:
			slog.Info("Secret deleted", "namespace", secret.ObjectMeta.Namespace, "name", secret.ObjectMeta.Name)
