...
func httpserver(cp certProvider, sc secretChecker, sr secretRotator, burst int, delay int) *http.Server {
	httpRateLimiter := rateLimiter(burst, delay)
...
func rateLimiter(burst int, delay int) throttled.HTTPRateLimiter {
	store, err := memstore.New(65536)
	if err != nil {
		log.Fatal(err)
	}

	quota := throttled.RateQuota{MaxRate: throttled.PerSec(delay), MaxBurst: burst}
