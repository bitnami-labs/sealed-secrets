...
func TestMainError(t *testing.T) {
	...
	err := runKubeseal(io.Discard, testConfig(&flags))
...
`)
	...
	if err := runKubeseal(&buf, testConfig(&flags)); err != nil {
		t.Fatal(err)
	}
...
func Test_runKubeseal(t *testing.T) {
	type args struct {
		cfg *Config
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			w := &bytes.Buffer{}
			if err := runKubeseal(w, tt.args.cfg); (err != nil) != tt.wantErr {
				t.Errorf("runKubeseal() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if gotW := w.String(); gotW != tt.wantW {
				t.Errorf("runKubeseal() = %v, want %v", gotW, tt.wantW)
			}
		})
	}
}
...
func trySealTestItem(certFilename, secretNS, secretName, secretValue string, scope ssv1alpha1.SealingScope) (string, error) {
	...
	if err := runKubeseal(&buf, cfg); err != nil {
		return "", err
	}
